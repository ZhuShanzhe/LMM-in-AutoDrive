...
from scene3_vla_controller import (
    ...
    apply_scene3_timed_resume_gate,
    assess_scene3_risk,
...
class Scene3VlaControllerTests(unittest.TestCase):
    ...
    def test_timed_resume_requires_worker_command_and_completed_hold(self):
        stopped = {
        ...
        before, override = apply_scene3_timed_resume_gate(
            stopped,
            risk,
            command_id="scene3_worker_crossing",
            stationary_elapsed_s=2.9,
            resume_active=False,
        )
        self.assertIsNone(override)
        self.assertEqual(before["action"], "emergency_brake")
        resumed, override = apply_scene3_timed_resume_gate(
            stopped,
            risk,
            command_id="scene3_worker_crossing",
        ...
        self.assertEqual(override, "cleared_worker_resume")
        self.assertEqual(resumed["action"], "keep_lane")
        self.assertEqual(resumed["target_speed_kmh"], 15.0)
        self.assertFalse(resumed["emergency"])

    def test_timed_resume_never_overrides_renewed_risk_or_other_command(self):
        stopped = {"action": "emergency_brake", "target_speed_kmh": 0.0}
        for command_id, risk_level in (
            ("scene3_worker_crossing", "high"),
            ("scene3_cruise", "low"),
        ):
            decision, override = apply_scene3_timed_resume_gate(
                stopped,
                {"risk_level": risk_level},
                command_id=command_id,
                stationary_elapsed_s=30.0,
                resume_active=True,
            )
            self.assertIsNone(override)
            self.assertEqual(decision["action"], "emergency_brake")
