...
def load_config(path: Path) -> dict[str, Any]:
    ...
    interfaces = payload.get("decision_interfaces", {})
    if interfaces.get("simulation_frame_is_sync_key") is not True:
        raise ValueError("Scene 2 decision inputs must use the simulation frame sync key")
    if interfaces.get("carla_truth_allowed_for_policy") is not False:
        raise ValueError("CARLA truth must not be exposed to the Scene 2 policy")
