...
- 仓库主 README 的场景二验收：8 km、违规不超过 1、无碰撞、ASR 不低于
  96%、组合步骤无遗漏、语义对齐不低于 98.5%；
- `docs/competition_voice_scenario_design.md` 的 6 段路线、混合交通与事件顺序；
...
| 竞赛指标 | ASR ≥96%；组合步骤遗漏为 0；语义对齐 ≥98.5% |

线上 8 km 单次运行只能证明路线、安全和接口闭环。ASR 与语义对齐准确率必须用
独立离线语料统计，不能由运行器伪造为成功。
