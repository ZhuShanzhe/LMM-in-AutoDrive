...
class ScriptedWalker:
    def __init__(
        ...
        *,
        ...
        hesitation_at_ratio: float | None = None,
        hesitation_ticks: int = 0,
    ) -> None:
        self.actor = actor
        self.target = target
        ...
        self.start_delay_ticks = max(0, int(start_delay_ticks))
        self.hesitation_at_ratio = hesitation_at_ratio
        self.hesitation_ticks = max(0, int(hesitation_ticks))
        self._ticks = 0
        self._initial_distance: float | None = None
        self._hesitation_remaining = 0
        self._hesitated = False
    ...
    def update(self) -> None:
        ...
        self._ticks += 1
        if self._ticks <= self.start_delay_ticks:
            self.actor.apply_control(
                carla.WalkerControl(direction=carla.Vector3D(), speed=0.0)
            )
            return
        travelled_ratio = 1.0 - distance / self._initial_distance
        if (
            not self._hesitated
            and self.hesitation_at_ratio is not None
            and travelled_ratio >= float(self.hesitation_at_ratio)
        ):
            self._hesitated = True
            self._hesitation_remaining = self.hesitation_ticks
        if self._hesitation_remaining > 0:
            self._hesitation_remaining -= 1
            self.actor.apply_control(
                carla.WalkerControl(direction=carla.Vector3D(), speed=0.0)
            )
            return
...
class DeterministicSceneEvents:
    ...
    def __init__(
        ...
        self._slow_vehicle_reacted = False
        self._bus_departed = False
        self._cyclist_profile_index = -1
        self._spawned = False
    ...
    def _spawn_walker(
        ...
        *,
        ...
        return ScriptedWalker(
            ...
            target,
            speed_mps,
            start_delay_ticks=int(behavior.get("start_delay_ticks", 0)),
            hesitation_at_ratio=behavior.get("hesitation_at_ratio"),
            hesitation_ticks=int(behavior.get("hesitation_ticks", 0)),
        )
    ...
    def spawn(self) -> None:
        ...
        self.scripted_walkers["crosswalk_pedestrian"] = (
            self._spawn_walker(
                ...
                behavior=crossing_config.get("behavior"),
            )
        )
        ...
        for index, longitudinal in enumerate((-5.0, 5.0, 8.0)):
            ...
            self.scripted_walkers[key] = self._spawn_walker(
                ...
                passenger_speed,
                behavior={
                    "start_delay_ticks": index * int(
                        bus_config.get("passenger_headway_ticks", 12)
                    )
                },
            )
    ...
    def update(self, progress_m: float) -> list[dict[str, Any]]:
        ...
        by_kind = {str(event["kind"]): event for event in self.events}
        slow_config = by_kind.get("slow_vehicle", {})
        brake_pulse = slow_config.get("brake_pulse", {})
        if (
            self.slow_vehicle is not None
            and isinstance(brake_pulse, Mapping)
            and not self._slow_vehicle_reacted
            and progress_m >= float(brake_pulse.get("trigger_progress_m", math.inf))
        ):
            self._set_desired_speed(
                self.slow_vehicle,
                float(brake_pulse.get("minimum_speed_kmh", 8.0)),
                85.0,
            )
            self._slow_vehicle_reacted = True

        cyclist_config = by_kind.get("cyclist", {})
        speed_profile = cyclist_config.get("speed_profile", [])
        if self.cyclist is not None and isinstance(speed_profile, list):
            for index, item in enumerate(speed_profile):
                if (
                    isinstance(item, Mapping)
                    and progress_m >= float(item.get("at_progress_m", math.inf))
                    and index > self._cyclist_profile_index
                ):
                    self._set_desired_speed(
                        self.cyclist,
                        float(item["target_speed_kmh"]),
                        75.0,
                    )
                    self._cyclist_profile_index = index
        ...
        if (
            ...
            and self.bus_active_ticks >= int(
                by_kind.get("bus_stop", {}).get("minimum_dwell_ticks", 80)
            )
            and self.states["bus_stop_passengers"] == "ACTIVE"
        ):
            if self.bus is not None and not self._bus_departed:
                self.bus.apply_control(carla.VehicleControl(hand_brake=False))
                self.bus.set_autopilot(True, self.traffic_manager.get_port())
                self._set_desired_speed(
                    self.bus,
                    float(by_kind.get("bus_stop", {}).get("departure_speed_kmh", 18.0)),
                    65.0,
                )
                self.traffic_manager.auto_lane_change(self.bus, True)
                self._bus_departed = True
        ...
        for event in self.events:
            event_id = str(event["id"])
            if self.states[event_id] != "ACTIVE":
                continue
            kind = str(event["kind"])
            if kind in {"slow_vehicle", "cyclist"} and progress_m >= float(
                event.get("resolve_progress_m", event.get("anchor_progress_m", 0.0) + 450.0)
            ):
                self.states[event_id] = "RESOLVED"
                changes.append(
                    {"event_id": event_id, "state": "RESOLVED", "progress_m": progress_m}
                )
        return changes

    def observation(self, progress_m: float) -> dict[str, Any]:
        """Stable interface for perception/VLA logs without CARLA truth values."""
            ...
            "events": [
                {
                    "event_id": str(event["id"]),
                    "kind": str(event["kind"]),
                    "lifecycle_state": self.states[str(event["id"])],
                    "expected_modalities": list(
                        event.get("expected_modalities", ["vision", "vehicle_state"])
                    ),
                }
                for event in self.events
            ],
            "policy_use": "evaluation_and_logging_only",
        }
