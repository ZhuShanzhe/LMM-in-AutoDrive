...
class BrakingWithTrafficValidationScenario(EmergencyBrakeScenario):
    ...
    def create_temporary_policy(
        ...
        """Provide one stable intent for VLA safety-gate integration tests."""
        from control.voice_schedule_policy import VoiceSchedulePolicy

        return VoiceSchedulePolicy(
            [{
                "id": "vla_follow_braking_lead",
                "announce_at_m": 0.0,
                "action": "keep_lane",
                "target_speed_kmh": float(default_speed_kmh),
                "voice_text": "Keep the current lane and drive at 35 km/h.",
                "parser_text_en": "Keep the current lane and drive at 35 km/h.",
            }],
            default_speed_kmh,
            parser_model_path,
            parser_device,
