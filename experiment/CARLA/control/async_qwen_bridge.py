...
class AsyncQwenBridge:
    ...
    def __init__(
        ...
        *,
        ...
        self.probe_path = self.output_dir / "qwen_async_probes.jsonl"
    ...
    def submit_capture(self, capture: Mapping[str, Any]) -> dict[str, Any] | None:
        ...
        probe = {
            ...
            "model_input_keys": ["image_path", "prompt"],
            "fusion_metadata_keys": ["world_state_path", "projection_path"],
            "ground_truth_object_count": len(record.get("ground_truth_objects", [])),
        }
        append_jsonl(self.probe_path, probe)
    ...
    def poll(self) -> dict[str, Any] | None:
        ...
        if result.get("status") != "valid" or capture is None:
            append_jsonl(self.probe_path, {
                "type": "qwen_output", "frame_id": frame_id,
                "status": result.get("status"),
                "fused": False,
            })
            return response
        try:
            ...
            append_jsonl(self.probe_path, {
                "type": "qwen_output", "frame_id": frame_id,
                "status": "fusion_error", "fused": False,
                "error_type": type(exc).__name__,
            })
        return response

    def wait_for_result(self, timeout_s: float) -> dict[str, Any] | None:
        """Wait only during experiment finalization, then persist one result.

        This method is intentionally not used in the simulation tick.  It lets
        a short run retain its final asynchronous Qwen audit without allowing a
        stale keyframe to affect closed-loop vehicle control.
        """

        if timeout_s < 0:
            raise ValueError("timeout_s must be non-negative")
        self.worker.wait_for_result(timeout_s)
        return self.poll()
