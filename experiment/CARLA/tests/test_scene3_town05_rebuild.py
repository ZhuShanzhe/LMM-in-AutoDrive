...
class Town05Scene3ContractTests(unittest.TestCase):
    ...
    def test_two_lane_mapping_is_stable_across_route_reference_lane(self):
        left = Waypoint(-1, "Driving", 0.0, -3.5)
        right = Waypoint(-2, "Driving", 0.0, 0.0)
        left._right = right
        right._left = left
        official_map = mock.Mock()
        adapter = Town05RouteMapAdapter(
        ...
        for progress_m in (0.0, 100.0):
            self.assertIs(adapter.logical_waypoint(-1, progress_m), left)
            self.assertIs(adapter.logical_waypoint(-2, progress_m), right)
            self.assertIsNone(adapter.logical_waypoint(-3, progress_m))

    def test_work_zone_contract_narrows_two_lanes_to_one(self):
        config = json.loads(CONFIG.read_text(encoding="utf-8"))
        work_zone = next(
            event for event in config["events"] if event["id"] == "scene3_work_zone"
        )
        self.assertEqual(work_zone["zone"]["closed_lane_id"], -2)
        self.assertEqual(work_zone["zone"]["open_lane_ids"], [-1])
        self.assertEqual(work_zone["safety"]["minimum_open_lane_count"], 1)
