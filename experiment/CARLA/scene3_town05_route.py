...
def _driving_lane_family(waypoint: Any) -> list[Any]:
    """Return all same-direction driving lanes from left to right."""

    left: list[Any] = []
    current = waypoint
    while True:
        candidate = _adjacent_driving(current, "left")
    ...
    current = waypoint
    while True:
        candidate = _adjacent_driving(current, "right")
        if candidate is None:
            break
...
class Town05RouteMapAdapter:
    ...
    def logical_waypoint(self, logical_lane_id: int, progress_m: float) -> Any | None:
        route_waypoint = self.route_waypoint(progress_m)
        family = _driving_lane_family(route_waypoint)
        logical_lane_id = int(logical_lane_id)
        if logical_lane_id == LOGICAL_LEFT_LANE:
            return family[0] if len(family) >= 2 else None
        if logical_lane_id == LOGICAL_CENTRE_LANE:
            if len(family) <= 2:
                return family[-1]
            return family[len(family) // 2]
        if logical_lane_id == LOGICAL_RIGHT_LANE:
            return family[-1] if len(family) >= 3 else None
        if logical_lane_id == LOGICAL_SHOULDER_LANE:
            return _roadside_lane(route_waypoint, "shoulder") or _roadside_lane(
                route_waypoint, "sidewalk"
            )
    ...
    def traffic_manager_path(
        ...
        *,
        ...
        """Return a forward route path for non-event Traffic Manager cars."""

        if spacing_m <= 0.0:
            raise ValueError("traffic path spacing must be positive")
        locations: list[Any] = []
        next_progress_m = float(start_progress_m) + spacing_m
        for (route_waypoint, _road_option), progress_m in zip(
            self.route,
            self.distances_m,
        ):
            if float(progress_m) + 1e-6 < next_progress_m:
                continue
            selected = route_waypoint
            if not bool(getattr(route_waypoint, "is_junction", False)):
                selected = (
                    self.logical_waypoint(logical_lane_id, float(progress_m))
                    or self.logical_waypoint(
                        LOGICAL_CENTRE_LANE,
                        float(progress_m),
                    )
                    or route_waypoint
                )
            locations.append(selected.transform.location)
            next_progress_m = float(progress_m) + spacing_m
        final_location = self.route[-1][0].transform.location
...
def validate_scene3_event_anchors(
    ...
    for event in events:
        ...
        if scenario == "cut_in_vehicle":
            actor = event["actor"]
            context.adapter.validate_anchor(
                distance_m,
                (actor["spawn_lane_id"], actor["target_lane_id"]),
            )
        elif scenario == "maintenance_vehicle_blockage":
            blockage = event["blockage"]
            context.adapter.validate_anchor(
                distance_m,
                (blockage["lane_id"], blockage["target_lane_id"]),
            )
            ...
            if scenario == "progressive_lane_closure":
                ...
                required_lanes = (
                    section["closed_lane_id"],
                    section["merge_target_lane_id"],
                )
            else:
                section = event["zone"]
                ...
                required_lanes = (
                    section["closed_lane_id"],
                    *section["open_lane_ids"],
                )
            sample_m = start_m
            while sample_m <= end_m:
                context.adapter.validate_anchor(sample_m, required_lanes)
                sample_m += 50.0
