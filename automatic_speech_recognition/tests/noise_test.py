...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Noisy ASR dataset test")
    parser.add_argument("--dataset", default="data/wav_files/noise/standard_noise/mapping.json")
    ...
    parser.add_argument("--save-dir", default="outputs/tests/noise")
    ...
    parser.add_argument("--log-file", default="logs/tests/noise_test.log", help="run log path; empty string disables it")
...
def main() -> None:
    ...
    log_and_print(f"[noise_test] loaded {len(records)} records")
    ...
    log_and_print(f"[noise_test] devices: {devices}")
    ...
    lanes = [lambda p, s=svc: s.transcribe_file(p, use_frontend=False, use_dialect=False) for svc in services]
    summary = ASREvaluator(lanes[0], records, enable_slots=args.enable_slots, transcribes=lanes).run(save_dir=str(save_dir), label="noise_test", **run_kwargs)

    log_and_print(report(summary, "Noisy ASR Test"))
