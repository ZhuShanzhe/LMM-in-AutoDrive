...
try:
    import torch
except ModuleNotFoundError:  # Contract validation remains usable without ML runtime.
...
class SensorTensorBatch:
    ...
    def validate(self) -> None:
        if torch is None:
            raise RuntimeError(
                "PyTorch is required to validate tensor batches, but JSON "
                "decision contracts remain available without it"
            )
        tensors = {
