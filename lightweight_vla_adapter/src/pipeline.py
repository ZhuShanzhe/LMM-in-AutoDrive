...
from .decision_coordinator import VLAFirstDecisionCoordinator
...
class LightweightVLAPipeline:
    def __init__(
        ...
        *,
        ...
        decision_coordinator: VLAFirstDecisionCoordinator | None = None,
        decision_mode: str = "vla_first",
    ) -> None:
        if decision_mode not in {"vla_first", "legacy_fsm"}:
            raise ValueError("decision_mode must be 'vla_first' or 'legacy_fsm'")
        ...
        self.decision_coordinator = (
            decision_coordinator or VLAFirstDecisionCoordinator()
        )
    ...
    def reset_temporal_state(self, stream_id: str | None = None) -> None:
        ...
        self.decision_coordinator.reset(stream_id)
    ...
    def decide(
        ...
        *,
        ...
        input_health: dict[str, Any] | None = None,
        ...
        if mode == "legacy_fsm":
            ...
            if prior_state is not None:
                if prior_state.get("schema_version") != "1.0.0" or (
                    "completed_step_ids" not in prior_state
                ):
                    raise ValueError(
                        "vla_first mode requires DecisionCoordinatorState as prior_state"
                    )
                self.decision_coordinator.restore(prior_state)
            state, final_decision = self.decision_coordinator.coordinate(
                proposal,
                driving_intent,
                world_state,
                semantic_alignment,
