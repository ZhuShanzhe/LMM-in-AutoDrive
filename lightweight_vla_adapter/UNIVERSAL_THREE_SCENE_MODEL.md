...
- 场景一 CARLA 实测：**5 km 全程通过**（r18，`task_completed=true`，0 碰撞、0 非法车道、15/15 指令、横偏 0.083 m）；
- 场景二 8 km 路线跑完但含 15 次碰撞/85 次实线侵入，未达严格通过标准；
- 场景三 r25/r29/r30 三次通过 6 km 全程（最近 r30 为动态转弯协同复跑：7/7 事件、0 碰撞、0 实线违规、0 fallback）；stage-4 与 stage-3 同框架同结构，stage-4 主要改善 Town04 前视误报。

## 通用控制修复（2026-08-08 第二轮/第三轮：动态转弯协同）

- 转弯参数不再写死：转向限幅由实际车速、路线曲率、车道宽度与航向误差实时计算（自行车模型 + 2.2 m/s² 横向舒适上限），并随车速自适应低通滤波；
- 转弯过程持续采样：每个控制步用横摆角速度（无横摆率时用自行车模型 + 最近实际转角）估计当前轨迹曲率，与路线参考曲率比较；车辆位于弯道外侧时向弯心补转，位于内侧时回正抑制并联动限速（12–24 km/h），形成“采样轨迹—估计偏差—持续调整”闭环；
- 路口限速随规划走廊实际转角动态计算（急弯约 7–9 km/h，缓弯可至 14 km/h），路口前瞻随车速 2.8–4.5 m 自适应，避免弯前切内；
