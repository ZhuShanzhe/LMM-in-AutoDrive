...
当前 `challenge-track` 已加入事件记忆、3 秒速度/加速度序列与显式低层跟踪，原默认链路保留。组员以 [序列策略联调说明](CHALLENGE_HANDOFF.md) 中的配置、输入输出、权重位置和适配边界为准。候选模型是实验版，非纵向指令仍走原路径；训练数据与中间权重不随此次提交分发。

新接口可在不下载权重的情况下运行 `python lightweight_vla_adapter/scripts/smoke_sequence_runtime.py`。候选权重不在 Git，不能认为拉取分支后自动具备真实模型推理能力。

## 原稳定链路说明

以下保留原稳定链路的架构、接口与历史结果；新增序列分支以本页上方联调说明为准。本次仅新增运行核心、配置与接口回归，未纳入个人工作区新增的训练、数据生成和中间实验文件。历史性能不代表新序列模型性能。
