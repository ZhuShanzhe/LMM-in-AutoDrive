...
从仓库根目录执行 `source submission_env.sh`。最终运行权重位于：

```text
models/lightweight_vla_adapter/v10/model.pt
models/modernbert-drive-command-compositional/
```

分别通过 `VLA_MODEL_PATH` 和 `MODERNBERT_MODEL_PATH` 传给推理入口。完整提交包
直接包含权重、模型卡、许可证和校验文件；普通 Git 分支不跟踪大模型文件。
...
MODEL_DIR=models/lightweight_vla_adapter/v10
mkdir -p "$MODEL_DIR"
...
python lightweight_vla_adapter/scripts/train_student.py \
  ...
  --output models/lightweight_vla_adapter/full_training_v7/model.pt \
  --metrics-output models/lightweight_vla_adapter/full_training_v7/train_metrics.json \
  --init-checkpoint models/lightweight_vla_adapter/carla_adapted_v6/model.pt \
  --epochs 8 \
  --batch-size 256 \
  --learning-rate 5e-6 \
  --weight-decay 0.01 \
...
python lightweight_vla_adapter/scripts/run_offline_inference.py \
  --config lightweight_vla_adapter/configs/student_base.json \
  --checkpoint models/lightweight_vla_adapter/v10/model.pt \
...
python lightweight_vla_adapter/scripts/benchmark_end_to_end_latency.py \
  --config lightweight_vla_adapter/configs/student_base.json \
  --checkpoint models/lightweight_vla_adapter/v10/model.pt \
  --language-model models/modernbert-drive-command-compositional \
