...
> 2026-08-03 起默认运行模式为 `vla_first`：VLA 负责正常行为决策，
> `VLAFirstDecisionCoordinator` 负责复合步骤、机动记忆、多源健康检查和动态重规划，
> 确定性逻辑仅保留安全约束。原 FSM 路径通过 `decision_mode="legacy_fsm"` 保留。
> 完整接口与迁移说明见
> [场景与决策更新文档](../docs/SCENE_DECISION_UPDATE_2026-08-03.md)。
